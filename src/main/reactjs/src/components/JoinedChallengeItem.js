...
const JoinedChallengeItem = ({challenge}) => {
  ...
  return (
    <div className={styles.profile_challenge}>
      ...
      <div className={styles.info_wrap}>
        ...
        <div>
          {
            new Date(challenge.planned_at) <= new Date() ? <span className={`${styles.status} ${styles.gray}`}>성공</span> : <span className={`${styles.status} ${styles.activate}`}>예정</span>
          }
        </div>
      </div>
    </div>
...
export default JoinedChallengeItem;